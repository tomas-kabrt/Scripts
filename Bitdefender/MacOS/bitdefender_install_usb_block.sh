...
if [ -e /Library/Bitdefender/AVP/EndpointSecurityforMac.app ]; then
  #This is a small fix for discrepancy between <7.4.x and 7.6.x - BD changed the location of the app - it could be deleted in the future, when the version is consistent. The new location is "/Library/Bitdefender/AVP/product/bin/BDLDaemon.app"
  if pkgutil --check-signature "/Library/Bitdefender/AVP/EndpointSecurityforMac.app" | tr -d '[:space:]' | grep -q "$bitdefender_cert_installed"  || pkgutil --check-signature "/Library/Bitdefender/AVP/product/bin/BDLDaemon.app" | tr -d '[:space:]' | grep -q "$bitdefender_cert_installed"; then
...
if [ "$arch_name" = "x86_64" ]; then
    # "Running on Intel"
    dl_link="https://cloudgz.gravityzone.bitdefender.com/Packages/MAC/0/tDlqxk/Bitdefender_for_MAC.dmg"
    pkg_name="antivirus_for_mac.pkg"
elif [[ "$arch_name" =~ "arm" ]]; then
    # "Running on ARM"
    dl_link="https://cloudgz.gravityzone.bitdefender.com/Packages/MAC/0/tDlqxk/Bitdefender_for_MAC_ARM.dmg"

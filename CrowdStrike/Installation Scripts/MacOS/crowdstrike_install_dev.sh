...
#BITDEFENDER_PW: Password for uninstallation of Bitdefender

CLIENT_ID=''
CLIENT_SECRET=''
BASE_URL=''
FILE_NAME='FalconSensorMacOS.LionLanner.pkg'
CS_CCID=''
CS_INSTALL_TOKEN=
...
if [ ! -x "/Applications/Falcon.app/Contents/Resources/falconctl" ] || [ -z "$(/Applications/Falcon.app/Contents/Resources/falconctl stats | grep 'Sensor operational: true')" ]; then
    ...
    curl -o /tmp/${FILE_NAME} -s -H "Authorization: Bearer ${APITOKEN}" ${BASE_URL}/sensors/entities/download-installer/v1?id=${FALCON_LATEST_SHA256} || exit 1
    installer -verboseR -package /tmp/${FILE_NAME} -target / || exit 1
    rm /tmp/${FILE_NAME}
		/Applications/Falcon.app/Contents/Resources/falconctl grouping-tags set "developer" || exit 1

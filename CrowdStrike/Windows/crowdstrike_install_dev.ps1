...
https://github.com/CrowdStrike/falcon-scripts/blob/main/powershell/install/falcon_windows_install.ps1
...
.PARAMETER UninstallBDPassword
Password for BD uninstallation
.PARAMETER Verbose
...
begin {
    ...
    function Check_BD {
		if(Get-Service EPProtectedService -ErrorAction SilentlyContinue){
			Write-FalconLog "EPProtectedService exists"  > $null
			return $true
		} else {
			Write-FalconLog "EPProtectedService does not exist" > $null
			return $false
		}
    }
...
process {
    ...
    if (Check_BD) {
        ...
        $cmd_params = "/bdparams /password=`"$UninstallBDPassword`" -noWait"

        try {
            # Download the file to the temp directory
            Write-FalconLog 'Downloading the Bitdefender Uninstaller'
            ...
            Write-FalconLog 'Bitdefender Uninstaller hash: ' $executableCertHash

            if ($executableCertHash -eq "09F279D3828B8FD31B771A7596D702198739D180") {
                # Run the uninstall tool with params and silently
                $process = Start-Process -FilePath $tempDir -ArgumentList $cmd_params -NoNewWindow -Wait
            } else {
                Write-FalconLog "Cert for Downloaded Bitdefender Uninstallator doesn't match"
                throw "Cert for Downloaded Bitdefender Uninstallator doesn't match"
            }

            Start-Sleep -s 10

            if (Check_BD) {
                Write-FalconLog "Bitdefender is still on the endpoint, bailing from the Falcon installation."
                throw "Bitdefender is still on the endpoint, bailing from the Falcon installation."
            }

        } catch {
            $message = $_
            Write-FalconLog 'Bitdefender Uninstallation ' $message
            throw $message
        }
    }

    Write-FalconLog 'Starting Falcon Installation'
